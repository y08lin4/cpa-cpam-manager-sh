...
`/health`、`/status`、`/v1/models`、管理页面和静态资源只计入过滤数量，不进入任何榜单；无法分类的路径和不带明确方法/路径的模糊日志只显示诊断数量，也不会混入消费或管理审计。HTTP 2xx 计为成功，HTTP 4xx/5xx 计为失败。公网、内网和回环地址都会在本地统计，401、403 会在所属行为审计中单独提示。
